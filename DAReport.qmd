...
author: 
  - Trifanov Matvei
  - Pasindu Perera
abstract: "This project analyzed flight data from New York City airports to identify key factors contributing to flight delays and develop predictive models for both regression and classification purposes. Leveraging five datasets encompassing flight details, weather conditions, airline information, and other relevant attributes, we undertook comprehensive data cleaning, preprocessing, and exploratory analysis to ensure data reliability and derive meaningful insights. The regression analysis focused on forecasting flight arrival delays (arr_delay) using variables such as flight distance, weather conditions, and airline carriers. By evaluating the regression model with metrics like RMSE and MAE, we gained insights into significant contributors to delays, though the data's inherent variability presented challenges. Additionally, a classification model was developed to categorize flights, offering complementary insights into the dataset. This project illustrates the application of supervised learning methodologies to complex, real-world datasets, shedding light on both the potential and limitations of predictive analytics in addressing multifaceted problems."
format:   
  html: 
    toc: true
    toc-depth: 3
...
|                                        |
|:---------------------------------------|
| SOMEONE EDIT THIS SECTION IF NECESSARY |

This Project works with dataframes about flights, specifically a sample of domestic flights that departed from the three major New York City airports in 2013.

There are 5 tabular data frames.
In total there is information about\
...
In this project, we adopted a structured approach to handle missing values across the datasets, carefully balancing the trade-off between preserving data and maintaining noisiness.
Specifically, for datasets with multiple variables containing missing values, such as the `flights` and `weather`, we established a threshold.

1.  Remove rows where all critical variables are missing: These rows provide no usable information and contribute little to the analysis.

2.  Impute rows with partial missing values: For rows where only a few variables are missing (e.g., less than 50%), we applied imputation to preserve the remaining information.

```{r}
#| label: missing_data

#####
##### THIS CHUNK IS COMPLETE. NO NEED TO CHANGE
#####
...
This approach strikes a good balance between preserving data and maintaining quality, ensuring that incomplete yet valuable rows are not discarded unnecessarily while avoiding excessive imputation for highly incomplete rows.

|                                             |
|:--------------------------------------------|
| IDK BUT WE CAN ALSO MENTION THE STEPS BELOW |
...
get_mode = function(x) {
  ux = unique(na.omit(x))  # Removes NA + gets unique
  ux[which.max(tabulate(match(x, ux)))]  # fins the most frequent value
}

###
### Flights dataframe. Inspecting and cleaning
###
# Exploring and Cleaning Missing Values

# Count missing values for each column
print(colSums(is.na(flights)))

# Check patterns for missing dep_time or arr_time
missing_critical <- flights %>% 
  filter(is.na(dep_time) | is.na(arr_time))

print("Subset of Rows with Missing dep_time or arr_time: ")
print(head(missing_critical))

# Calculate the proportion of rows with missing critical columns
total_rows <- nrow(flights)
missing_critical_rows <- nrow(missing_critical)
...
print("Final Missing Data Counts After Cleaning: ")
print(missing_count_final)

# Check the row counts before and after cleaning
cat("Original Rows: ", nrow(flights), "\n")
cat("Cleaned Rows: ", nrow(flights_cleaned), "\n")

# we check what columns have missing values
# then we apply the aforementioned approach
...
cat("Missing values after cleaning airports dataset:\n")
...
### Planes cleaning
###
colSums(is.na(planes)) # 89 out of 4840 observations are missing
...
cat("The median year is:", median_year, "\n")

# Impute missing 'year' with the median value
planes_clean = planes
planes_clean$year[is.na(planes_clean$year)] = median_year

cat("Missing values after cleaning planes dataset:\n")
colSums(is.na(planes_clean))
...
cat("Missing values after cleaning weather dataset:\n")
...
colSums(is.na(weather_clean)) # full cleaned
```

|                                                  |
|--------------------------------------------------|
| CONCLUSION FROM CLEANING + MAYBE SOME TABLES TO  |
| SHOW THAT WE DONT HAVE ANY MISSING VALUES NO MO! |
...
|                                     |
|:------------------------------------|
| INTRO FOR EDA. LIKE WHY DO WE DO IT |
| WHAT WE WANNA SEE AND ETC           |
...
flights_cleaned %>%
  ...
  annotate(geom = "label", x = 4, y = 0.25, 
           ...
           fill = "#7D7D7D", color = "white", fontface = "bold") +  # Annotation with styling
  labs(
  ...
  scale_fill_manual(
    values = c("1" = "#E74C3C", "0" = "#2E8B57"),  # Custom colors: Orange-red for late, blue-green for on-time
  ...
  theme(
    ...
    panel.grid.major = element_line(color = "gray80", linetype = "dashed"),  # Subtle gridlines
...
flights_cleaned %>%
  ...
  geom_line(color = "#0073C2", size = 1.2) +  # Line plot with custom color and size
  geom_point(size = 3, color = "#E74C3C") +  # Add points for emphasis
...
ggcorrplot(
  ...
  colors = c("#1B9E77", "white", "#D95F02"),  # New color gradient: green to orange
...
# Perform a left join to retain all rows from flights_cleaned
flights_weather = flights_cleaned |>
...
# Filter out rows with missing weather data (if necessary)
...
# Normalize the relevant numerical features without modifying the original data
...
# Evaluate the refined model on the test data
