...
```{r}
#| label: data_cleaning
# Inspecting and Cleaning Flights Data

# 1. Exploring Missing Values
missing_count_flights <- colSums(is.na(flights))
print(missing_count_flights)

# Visualizing missing data patterns using an upset plot
gg_miss_upset(flights)

# 2. Identifying Rows with Missing Critical Columns (dep_time or arr_time)
...
# Calculate and print the number and percentage of rows with missing critical columns
total_rows <- nrow(flights)
missing_critical_rows <- nrow(missing_dep_arr)
proportion_missing <- (missing_critical_rows / total_rows) * 100

print(paste("Number of Rows Missing dep_time or arr_time: ", missing_critical_rows))
print(paste("Percentage of Rows with Missing Critical Data: ", round(proportion_missing, 2), "%"))

# 3. Correlation of Missing Values
flights %>% 
  mutate(
...
## 4.1 Removing Rows with Missing Critical Columns (dep_time or arr_time)
...
# Impute missing arr_delay and air_time with the median value
...
# Departure Delay Distribution After Imputation
...
# Arrival Delay Distribution After Imputation
...
# Air Time Distribution After Imputation
...
# 6. Handling Missing tailnum and dep_delay

## 6.1 Remove Rows with Missing tailnum
...
## 6.2 Remove Rows with Missing dep_delay
...
# Departure Delay Distribution Before and After Removing Missing dep_delay
...
#structure of the weather data set
str(weather)
...
missing_count_weather <- colSums(is.na(weather))
print("Missing values in each column: ")
print(missing_count_weather)


# Visualize missing values
gg_miss_upset(weather) # Missing values upset plot
```
